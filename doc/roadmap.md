...
> 2026-09-21 范围校准：本文为较早路线讨论记录。当前 Demo 仅执行一次计划，不构建 Replan，M3 的重规划和 M7 的反馈再经营不再是 M4 前置条件或首版验收要求。以 AGENTS.md 和各 Demo V1 文档为准。M4 最新设计入口为 [定向洞察 Demo V1](design/2026-09-21-directional-insight-demo-v1.md)，其中用户已确认只输出一个优先经营机会；其余待确认内容不自动成为施工指令。下文旧字段和阶段完成描述不作为当前实现事实。
