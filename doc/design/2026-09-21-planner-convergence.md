...
> 最新范围：当前 Demo 不构建或接入 Replan，只生成并执行一次计划，完成 Goal 分析、机会发现、圈客和策略执行。本文有关首次规划与 Replan 共用基础及多版本治理均为后续参考，不属于当前任务。见 AGENTS.md 与 Planner Demo V1。

> 首版开发请以 [Planner Demo V1 范围与开发说明](2026-09-21-planner-demo-v1.md) 为准。本文保留为后续设计参考；其中完整规划结果、步骤身份映射、Planning Context、Capability View、静态输入路径校验、语义重试和先拆包后迁移，不再是首版必须完成的要求。本文中的“首版”“已确认事项”和验收清单不得绕过新文档扩大本次开发范围。该范围调整不代表代码已实现。
