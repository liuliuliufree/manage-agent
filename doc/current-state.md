...
- 最近验证基线：2026-09-21，收敛后的领域契约、M2-Lite Application 与 M3-T01 至 T07 共 76 项 `unittest` 通过，并完成 `src`、`tests` 及 `scripts` 字节码编译；FakeModel 与 Fake Capability 已离线覆盖正常多步完成、`NO_RESULT → Plan V2 → 继续执行 → FINISH`、成功步骤不重跑、缺信息询问、规则阻断停止和技术失败停止。8 个动态规划核心 Demo 场景仍由 FakeModel 离线覆盖。本轮未重新执行真实模型验证；此前 Goal Parser 三个场景和 Planner 四个首次规划动态选择场景已在线通过，Replan 尚未执行真实模型验证。BusinessAgent 修复后的最近一次在线复验在第一个模型调用处明确返回 30 秒超时，尚未重新取得完整在线通过结果。
