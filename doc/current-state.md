...
- React 前端以居中输入页进入左右对话流，按内部 Turn 归属组织模型公开说明和实际 Tool，但不向用户暴露 Turn 概念；Tool 详情默认折叠，最终回答支持流式 Markdown 渲染。
...
2026-09-16：12 项数据、确定性业务能力、Tool 自动发现、Agent 自主调用、事件生命周期和真实 HTTP SSE 契约测试通过；前端 TypeScript 检查与 Vite 生产构建通过。页面改造后使用新的临时输出目录完成生产构建，并通过真实模型流验证多段公开说明、五次 Tool 调用、Markdown 最终回答和桌面及 390×844 移动视口；未覆盖仓库中的旧构建产物。
